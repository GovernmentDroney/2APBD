...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = (from emp in emps
            where emp.DeptNo == 30
            select emp).ToList(); 
        IEnumerable<Emp> query = result.OrderByDescending(emp => emp.Sal);
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var depts = Database.GetDepts();
         ...
         List<Emp> result = (from emp in emps
             where depts.Where(d => d.Loc == "CHICAGO").Select(d=>d.DeptNo).Contains(emp.DeptNo)
             select emp).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = from emp in emps group emp by emp.DeptNo into g
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = (from emp in emps
            ...
            into g
            select new { DeptNo = g.Key, AvgSal = g.Average(s => s.Sal) }).ToList();        
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = (from emp in emps
                join avgSal in
                    ...
                    on emp.DeptNo equals avgSal.DeptNo
                where emp.Sal > avgSal.AvgSal
                select emp.EName)
            .ToList(); 
